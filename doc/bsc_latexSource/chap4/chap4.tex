...
Gil este puternic abstractizatã, folosind în mod avansat template-urile C++ pentru a obþine o performanþã crescutã (o parte dintre calcule/instanþieri sunt realizate la compilare). Biblioteca permite accesul la pixelii unei imagini într-un mod care nu depinde de formatul concret al acesteia. În plus, sunt asigurate structuri de acces (iteratori) care sã utilizeze în mod eficient cache-ul procesorului (procesoarelor), astfel încât prelucrarea datelor sã fie realizatã cât mai rapid. 
...
\end{lstlisting}
\end{framed}
Deºi se poate observa cã folosirea primitivelor gil reduce din lizibilitatea codului, o datã ce sunt cunoscute funcþiile de bazã ale acestei biblioteci, citirea algoritmului devine mult mai uºoarã. Avantajul principal este descrierea extrem de generalã a operaþiilor. Practic, formatul imaginii nu a fost precizat explicit nicãieri în cadrul funcþiei, fiind furnizat prin intermediul parametrilor template SrcView ºi DstView (pentru formatul imaginii de intrare, respectiv de ieºire). Folosind aceastã tehnicã, algoritmul poate lucra practic cu orice format, fãrã a trebui realizate modificãri în implementarea sa. Tipul propriu-zis al imaginilor este determinat la compilare.
...
\subsubsection{Determinarea Hessian-ului ºi a extremelor locale}
Calculul hessian-ului (\ref{eq:chap2:sifthessian}) ºi determinarea locaþiei precise a extremelor locale (ecuaþia~\ref{eq:chap2:siftinterpolatedextr}) sunt implementate în cadrul clasei FastHessian, urmãrindu-se paºii de mai jos:
\begin{itemize}
\item Calculul determinantului Hessian-ului, pentru toate punctele eºantionate ale imaginilor din spaþiul scalãrilor. Numãrul de scalãri precum ºi frecvenþa eºantionãrii sunt parametrii ai algoritmului deciºi în momentul rulãrii acestuia. (funcþia buildDet);
\item Eliminarea punctelor care nu sunt extreme locale în spaþiul scalãrilor;
\item Determinarea prin interpolare a poziþiei exacte a extremului în spaþiul scalãrilor, pentru punctele rãmase. (funcþia interpolateExtremum, ce implementeazã ecuaþia~\ref{eq:chap2:siftinterpolatedextr}) 
\end{itemize}
\begin{framed}
\begin{lstlisting}
...
Având în vedere cã se lucreazã pe mai multe fire de execuþie, se pune în mod natural întrebarea legatã de ordinea în care vor fi afiºate rezultatele procesãrii. O ordonare precisã a cadrelor ar presupune menþinerea pentru fiecare cadru a unui timp al apariþiei în sistemul de prelucrare ºi afiºarea rezultatelor pe baza acestui timp. Nici aceastã soluþie nu este una perfectã, deoarece cadre apãrute anterior în sistem pot sã fie terminate de procesat dupã ce un alt cadru (apãrut dupã ele) a fost deja prelucrat ºi afiºat. De asemenea, ar fi necesare o serie de regiuni critice, care ar scãdea din performanþa algoritmului. 
